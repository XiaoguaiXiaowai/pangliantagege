...
*   **🤖 私域 RAG 智能助理 (AI Assistant)**
    *   **本地大模型驱动**：基于本地部署的私域 RAG (检索增强生成) 服务构建的个人专属智能助理。
    *   **实时连通性探测**：前端实现了与 RAG 后端的实时 Socket 心跳检测（支持动态 IP 代理），UI 动态反馈服务连接状态（黄灯探测、绿灯连接、红灯断开）。
    *   **沉浸式聊天交互**：高颜值玻璃拟态（Glassmorphism）对话气泡、打字机加载动画（Typing indicator），以及自动滚动和多轮对话上下文支持。
    *   **关联开源项目**：AI 后端推理及检索逻辑基于独立仓库实现，详见 👉 [`rag-ai-01` GitHub 仓库](https://github.com/XiaoguaiXiaowai/rag-ai-01)。
